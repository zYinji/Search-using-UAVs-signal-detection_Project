...
%             obj.signalPos = [400 700 0];% 先放这
        ...
        %%%%%%%%%%%%%%%%%%%Change Position 1矩形寻找%%%%%%%%%%%%%%%%%%%%
        function obj = change_pos_and_orientation_rectangle(obj) 
            if (obj.round == 1)
                obj.pos(1) = obj.pos(1) + obj.speed * obj.time_interval;
                obj.pos(2) = obj.pos(2);
                obj.pos(3) = obj.height;
                if obj.pos(1) - obj.posrecord >= 0.8*2*obj.S
                    obj.round = 2;
                end
            end
            ...
            if (obj.round == 2)
                ...
                obj.pos(3) = obj.height;
                if obj.pos(2) >= 3000 - 0.8*2*obj.S
                    obj.round = 3;
                    obj.posrecord = obj.pos(1);
                end
            end
            ...
            if (obj.round == 4)
                ...
                obj.pos(2) = obj.pos(2) - obj.speed * obj.time_interval;
                obj.pos(3) = obj.height;
                if obj.pos(2) <= 0 + 0.8*(2*obj.period-1+fix(obj.round/3)) * obj.S
                    obj.round = 1;
                    obj.posrecord = obj.pos(1);
                end
            end
...
%                 obj.detect = 0;
            ...
            elseif size(obj.distance,1) > 20 % 9 position or more(add one more below)
                ...
                if size(obj.distance,1) == 200
                    obj.estPos = maxLikelihood(obj.UAVRecord(1:end-3,1),obj.UAVRecord(1:end-3,2),obj.distance(1:end-3));
                    obj.complete = 1;
                    estHeight = mean(obj.UAVRecord(:,3)-sqrt(obj.distance.^2-(obj.UAVRecord(:,1) ...
                               -obj.estPos(1)).^2-(obj.UAVRecord(:,2)-obj.estPos(2)).^2));
        ...
        function judge = update(obj)
            ...
            if obj.detect == -1 %直线飞行
                ...
                obj = change_pos_and_orientation_rectangle(obj);
